package org.valkyrienskies.krunch.datastructures;
 ...
 * <p>
 * Not thread-safe. Attempting to use this concurrently from multiple threads will likely have catastrophic results
 * (read: JVM crashes).
 *
 * @author DaPorkchop_
 */
public class Int3HashSet implements AutoCloseable {
    ...
    protected static final int BUCKET_AXIS_BITS = 2;
        //the number of bits per axis which are used inside of the bucket rather than identifying the bucket
    protected static final int BUCKET_AXIS_MASK = (1 << BUCKET_AXIS_BITS) - 1;
    protected static final int BUCKET_SIZE =
        (BUCKET_AXIS_MASK << (BUCKET_AXIS_BITS * 2)) | (BUCKET_AXIS_MASK << BUCKET_AXIS_BITS) | BUCKET_AXIS_MASK;

    protected static long hashPosition(final int x, final int y, final int z) {
        return x * 1403638657883916319L //some random prime numbers
            + y * 4408464607732138253L
            + z * 2587306874955016303L;
    }

    protected static long positionFlag(final int x, final int y, final int z) {
        return 1L <<
            (((x & BUCKET_AXIS_MASK) << (BUCKET_AXIS_BITS * 2)) | ((y & BUCKET_AXIS_MASK) << BUCKET_AXIS_BITS) |
                (z & BUCKET_AXIS_MASK));
    }

    protected static long allocateTable(final long tableSize) {
        final long size = tableSize * BUCKET_BYTES;
        final long addr = PlatformDependent.allocateMemory(size); //allocate
    ...
    public Int3HashSet(int initialCapacity) {
        ...
        initialCapacity =
            1 << (Integer.SIZE - Integer.numberOfLeadingZeros(initialCapacity - 1)); //round up to next power of two
    ...
    public boolean add(final int x, final int y, final int z) {
        final long flag = positionFlag(x, y, z);
        final long bucket = this.findBucket(x >> BUCKET_AXIS_BITS, y >> BUCKET_AXIS_BITS, z >> BUCKET_AXIS_BITS, true);

        final long value = PlatformDependent.getLong(bucket + BUCKET_VALUE_OFFSET);
    ...
    public boolean contains(final int x, final int y, final int z) {
        final long flag = positionFlag(x, y, z);
        final long bucket = this.findBucket(x >> BUCKET_AXIS_BITS, y >> BUCKET_AXIS_BITS, z >> BUCKET_AXIS_BITS, false);

        return bucket != 0L //bucket exists
            && (PlatformDependent.getLong(bucket + BUCKET_VALUE_OFFSET) & flag) != 0L; //flag is set
    }

    protected long findBucket(final int x, final int y, final int z, final boolean createIfAbsent) {
        final long tableSize = this.tableSize;
        ...
        final long mask = tableSize - 1L; //tableSize is always a power of two, so we can safely create a bitmask like this
        final long hash = hashPosition(x, y, z);

        for (long i = 0L; ; i++) {
            final long bucketAddr = tableAddr + ((hash + i) & mask) * BUCKET_BYTES;

            if (PlatformDependent.getLong(bucketAddr + BUCKET_VALUE_OFFSET) ==
                0L) { //if the bucket value is 0, it means the bucket hasn't been assigned yet
                if (createIfAbsent) {
                    if (this.usedBuckets < this.resizeThreshold) { //let's assign the bucket to our current position
                        ...
                        this.resize();
                        return this
                            .findBucket(x, y, z, createIfAbsent); //tail recursion will probably be optimized away
                    }
            ...
            if (PlatformDependent.getInt(bucketAddr + BUCKET_KEY_OFFSET + KEY_X_OFFSET) == x
                ...
                && PlatformDependent.getInt(bucketAddr + BUCKET_KEY_OFFSET + KEY_Z_OFFSET) ==
                z) { //we found the matching bucket!
                return bucketAddr;
            }
    ...
    protected void resize() {
        final long oldTableSize = this.tableSize;
        final long oldTableAddr = this.tableAddr;

        //allocate new table
        final long newTableSize = oldTableSize << 1L;
        this.setTableSize(newTableSize);
        final long newTableAddr = this.tableAddr = allocateTable(newTableSize);
        final long newMask = newTableSize - 1L;

        //iterate through every bucket in the old table and copy it to the new one
        for (long i = 0; i < oldTableSize; i++) {
            final long oldBucketAddr = oldTableAddr + i * BUCKET_BYTES;

            //read the bucket into registers
            final int x = PlatformDependent.getInt(oldBucketAddr + BUCKET_KEY_OFFSET + KEY_X_OFFSET);
            final int y = PlatformDependent.getInt(oldBucketAddr + BUCKET_KEY_OFFSET + KEY_Y_OFFSET);
            final int z = PlatformDependent.getInt(oldBucketAddr + BUCKET_KEY_OFFSET + KEY_Z_OFFSET);
            final long value = PlatformDependent.getLong(oldBucketAddr + BUCKET_VALUE_OFFSET);
            if (value == 0L) { //the bucket is unset, so there's no reason to copy it
                continue;
            }

            for (long hash = hashPosition(x, y, z), j = 0L; ; j++) {
                final long newBucketAddr = newTableAddr + ((hash + j) & newMask) * BUCKET_BYTES;

                if (PlatformDependent.getLong(newBucketAddr + BUCKET_VALUE_OFFSET) ==
                    0L) { //if the bucket value is 0, it means the bucket hasn't been assigned yet
     ...
     */
    public void forEach(final XYZConsumer action) {
        final long tableAddr = this.tableAddr;
        if (tableAddr == 0L) { //the table isn't even allocated yet, there's nothing to iterate through...
            return;
        }

        //haha yes, c-style iterators
        for (long bucket = tableAddr, end = tableAddr + this.tableSize * BUCKET_BYTES; bucket != end;
             bucket += BUCKET_BYTES) {
            //read the bucket into registers
            final int bucketX = PlatformDependent.getInt(bucket + BUCKET_KEY_OFFSET + KEY_X_OFFSET);
            final int bucketY = PlatformDependent.getInt(bucket + BUCKET_KEY_OFFSET + KEY_Y_OFFSET);
            final int bucketZ = PlatformDependent.getInt(bucket + BUCKET_KEY_OFFSET + KEY_Z_OFFSET);
            final long value = PlatformDependent.getLong(bucket + BUCKET_VALUE_OFFSET);
            ...
            for (int i = 0; i <= BUCKET_SIZE; i++) { //check each flag in the bucket value to see if it's set
                ...
                final int dx = i >> (BUCKET_AXIS_BITS * 2);
                final int dy = (i >> BUCKET_AXIS_BITS) & BUCKET_AXIS_MASK;
                final int dz = i & BUCKET_AXIS_MASK;
                action.accept((bucketX << BUCKET_AXIS_BITS) + dx, (bucketY << BUCKET_AXIS_BITS) + dy,
                    (bucketZ << BUCKET_AXIS_BITS) + dz);
            }
        }
    }
    ...
    public boolean remove(final int x, final int y, final int z) {
        ...
        final long mask =
            this.tableSize - 1L; //tableSize is always a power of two, so we can safely create a bitmask like this

        final long flag = positionFlag(x, y, z);
        final int searchBucketX = x >> BUCKET_AXIS_BITS;
        final int searchBucketY = y >> BUCKET_AXIS_BITS;
        final int searchBucketZ = z >> BUCKET_AXIS_BITS;
        final long hash = hashPosition(searchBucketX, searchBucketY, searchBucketZ);

        for (long i = 0L; ; i++) {
            final long bucketAddr = tableAddr + ((hash + i) & mask) * BUCKET_BYTES;

            //read the bucket into registers
            final int bucketX = PlatformDependent.getInt(bucketAddr + BUCKET_KEY_OFFSET + KEY_X_OFFSET);
            final int bucketY = PlatformDependent.getInt(bucketAddr + BUCKET_KEY_OFFSET + KEY_Y_OFFSET);
            final int bucketZ = PlatformDependent.getInt(bucketAddr + BUCKET_KEY_OFFSET + KEY_Z_OFFSET);
            final long value = PlatformDependent.getLong(bucketAddr + BUCKET_VALUE_OFFSET);
            if (value ==
                0L) { //the bucket is unset. we've reached the end of the bucket chain for this hash, which means
                return false;
            } else if (bucketX != searchBucketX || bucketY != searchBucketY ||
                bucketZ != searchBucketZ) { //the bucket doesn't match, so the search must go on
                continue;
            } else if ((value & flag) ==
                0L) { //we've found a matching bucket, but the position's flag is unset. there's nothing for us to do...
                return false;
            }
            ...
            if ((value & ~flag) ==
                0L) { //this position is the only position in the bucket, so we need to delete the bucket
                this.usedBuckets--;
    ...
    protected void shiftBuckets(final long tableAddr, long pos, final long mask) {
        long last;
        long slot;
        ...
        for (; ; ) {
            ...
            for (; ; pos = (pos + 1L) & mask) {
                final long currAddr = tableAddr + pos * BUCKET_BYTES;
                if ((currValue = PlatformDependent.getLong(currAddr + BUCKET_VALUE_OFFSET)) ==
                    0L) { //curr points to an unset bucket
                    PlatformDependent
                        .setMemory(tableAddr + last * BUCKET_BYTES, BUCKET_BYTES, (byte) 0); //delete last bucket
                    return;
                }
            ...
            final long lastAddr = tableAddr + last * BUCKET_BYTES;
    ...
    protected void setTableSize(final long tableSize) {
