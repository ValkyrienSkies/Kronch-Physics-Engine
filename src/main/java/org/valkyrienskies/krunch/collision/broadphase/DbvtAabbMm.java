...
public class DbvtAabbMm {
    ...
    public int Classify(final Vector3f n, final float o, final int s) {
        final Vector3f pi = new Vector3f(); // Stack.alloc(Vector3f.class);
        final Vector3f px = new Vector3f(); // Stack.alloc(Vector3f.class);
    ...
    public float ProjectMinimum(final Vector3f v, final int signs) {
        final Vector3f[] b = new Vector3f[] {mx, mi};
        final Vector3f p = new Vector3f(); // Stack.alloc(Vector3f.class);
        p.set(b[(signs >> 0) & 1].x,
            b[(signs >> 1) & 1].y,
            b[(signs >> 2) & 1].z);
        return p.dot(v);
    ...
    public static boolean Intersect(final DbvtAabbMm a, final DbvtAabbMm b, final Transform xform) {
        final Vector3f d0 = new Vector3f(); // Stack.alloc(Vector3f.class);
        final Vector3f d1 = new Vector3f(); // Stack.alloc(Vector3f.class);
        final Vector3f tmp = new Vector3f(); // Stack.alloc(Vector3f.class);
        ...
        transposeTransform(d1, d0, xform.basis);

        final float[] s0 = new float[] {0, 0};
        final float[] s1 = new float[2];
    ...
    public static boolean Intersect(final DbvtAabbMm a, final Vector3f org, final Vector3f invdir, final int[] signs) {
        final Vector3f[] bounds = new Vector3f[] {a.mi, a.mx};
        float txmin = (bounds[signs[0]].x - org.x) * invdir.x;
        float txmax = (bounds[1 - signs[0]].x - org.x) * invdir.x;
        final float tymin = (bounds[signs[1]].y - org.y) * invdir.y;
        final float tymax = (bounds[1 - signs[1]].y - org.y) * invdir.y;
        ...
        final float tzmin = (bounds[signs[2]].z - org.z) * invdir.z;
        final float tzmax = (bounds[1 - signs[2]].z - org.z) * invdir.z;
    ...
    public static void transposeTransform(final Vector3f dest, final Vector3f vec, final Matrix3f mat) {
        final float x = tdotx(mat, vec);
        final float y = tdoty(mat, vec);
        final float z = tdotz(mat, vec);
        dest.x = x;
        dest.y = y;
        dest.z = z;
    }
