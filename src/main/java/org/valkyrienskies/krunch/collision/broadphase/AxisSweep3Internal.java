...
/**
 * AxisSweep3Internal is an internal base class that implements sweep and prune. Use concrete implementation {@link
 * AxisSweep3} or {@link AxisSweep3_32}.
 *
 * @author jezek2
 */
public abstract class AxisSweep3Internal extends BroadphaseInterface {
    ...
    AxisSweep3Internal(final Vector3f worldAabbMin, final Vector3f worldAabbMax, final int handleMask,
        final int handleSentinel,
        final int userMaxHandles/* = 16384*/, final OverlappingPairCache pairCache/*=0*/) {
        this.bpHandleMask = handleMask;
        this.handleSentinel = handleSentinel;
        this.pairCache = pairCache;

        final int maxHandles = userMaxHandles + 1; // need to add one sentinel handle
    ...
    protected int allocHandle() {
        ...
        final int handle = firstFreeHandle;
    ...
    protected void quantize(final int[] out, final Vector3f point, final int isMax) {
        final Vector3f clampedPoint = new Vector3f(point); // Stack.alloc(point);

        VectorUtil.setMax(clampedPoint, worldAabbMin);
        VectorUtil.setMin(clampedPoint, worldAabbMax);

        final Vector3f v = new Vector3f(); //  Stack.alloc(Vector3f.class);
    ...
    protected void sortMinDown(final int axis, final int edge, final Dispatcher dispatcher,
        ...
        final EdgeArray edgeArray = pEdges[axis];
        int pEdge_idx = edge;
        int pPrev_idx = pEdge_idx - 1;

        final Handle pHandleEdge = getHandle(edgeArray.getHandle(pEdge_idx));

        while (edgeArray.getPos(pEdge_idx) < edgeArray.getPos(pPrev_idx)) {
            final Handle pHandlePrev = getHandle(edgeArray.getHandle(pPrev_idx));
    ...
    protected void sortMinUp(final int axis, final int edge, final Dispatcher dispatcher,
        ...
        while (edgeArray.getHandle(pNext_idx) != 0 && (edgeArray.getPos(pEdge_idx) >= edgeArray.getPos(pNext_idx))) {
            final Handle pHandleNext = getHandle(edgeArray.getHandle(pNext_idx));

            if (edgeArray.isMax(pNext_idx) != 0) {
                // if next edge is maximum remove any overlap between the two handles
                if (updateOverlaps) {
                    final Handle handle0 = getHandle(edgeArray.getHandle(pEdge_idx));
                    final Handle handle1 = getHandle(edgeArray.getHandle(pNext_idx));
        ...
        while (edgeArray.getPos(pEdge_idx) < edgeArray.getPos(pPrev_idx)) {
            ...
            if (edgeArray.isMax(pPrev_idx) == 0) {
                ...
                if (updateOverlaps) {
                    ...
                    final Handle handle0 = getHandle(edgeArray.getHandle(pEdge_idx));
                    final Handle handle1 = getHandle(edgeArray.getHandle(pPrev_idx));
                    pairCache.removeOverlappingPair(handle0, handle1, dispatcher);
                    if (userPairCallback != null) {
                        userPairCallback.removeOverlappingPair(handle0, handle1, dispatcher);
                    }
    ...
    public void calculateOverlappingPairs(final Dispatcher dispatcher) {
        if (pairCache.hasDeferredRemoval()) {
            final ObjectArrayList<BroadphasePair> overlappingPairArray = pairCache.getOverlappingPairArray();
            ...
            final BroadphasePair previousPair = new BroadphasePair();
            previousPair.pProxy0 = null;
            previousPair.pProxy1 = null;
            previousPair.algorithm = null;

            for (i = 0; i < overlappingPairArray.size(); i++) {
                final BroadphasePair pair = overlappingPairArray.getQuick(i);

                final boolean isDuplicate = (pair.equals(previousPair));
                ...
                if (!isDuplicate) {
                    final boolean hasOverlap = testAabbOverlap(pair.pProxy0, pair.pProxy1);
    ...
    public int addHandle(final Vector3f aabbMin, final Vector3f aabbMax, final Object pOwner,
        final short collisionFilterGroup,
        ...
        // allocate a handle
        final int handle = allocHandle();

        final Handle pHandle = getHandle(handle);
    ...
    public void removeHandle(final int handle, final Dispatcher dispatcher) {
        ...
        for (axis = 0; axis < 3; axis++) {
            final EdgeArray pEdges = this.pEdges[axis];
            final int max = pHandle.getMaxEdges(axis);
            pEdges.setPos(max, handleSentinel);

            sortMaxUp(axis, max, dispatcher, false);

            final int i = pHandle.getMinEdges(axis);
    ...
    public void updateHandle(final int handle, final Vector3f aabbMin, final Vector3f aabbMax,
        ...
        for (int axis = 0; axis < 3; axis++) {
            final int emin = pHandle.getMinEdges(axis);
            final int emax = pHandle.getMaxEdges(axis);

            final int dmin = (int) min[axis] - (int) pEdges[axis].getPos(emin);
            final int dmax = (int) max[axis] - (int) pEdges[axis].getPos(emax);
    ...
    public void destroyProxy(final BroadphaseProxy proxy, final Dispatcher dispatcher) {
        final Handle handle = (Handle) proxy;
