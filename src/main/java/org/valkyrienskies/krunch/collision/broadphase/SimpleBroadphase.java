...
public class SimpleBroadphase extends BroadphaseInterface {
    ...
    public BroadphaseProxy createProxy(final Vector3f aabbMin, final Vector3f aabbMax, final BroadphaseNativeType shapeType,
        ...
        final SimpleBroadphaseProxy proxy =
            new SimpleBroadphaseProxy(aabbMin, aabbMax, shapeType, userPtr, collisionFilterGroup, collisionFilterMask,
                multiSapProxy);
    ...
    public void calculateOverlappingPairs(final Dispatcher dispatcher) {
        for (int i = 0; i < handles.size(); i++) {
            final SimpleBroadphaseProxy proxy0 = handles.getQuick(i);
            for (int j = 0; j < handles.size(); j++) {
                final SimpleBroadphaseProxy proxy1 = handles.getQuick(j);
                if (proxy0 == proxy1) {
                    continue;
                }
