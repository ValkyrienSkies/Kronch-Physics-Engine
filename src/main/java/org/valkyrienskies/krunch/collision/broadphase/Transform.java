...
public class Transform {
    ...
    public void mul(final Transform tr) {
        final Vector3f vec = new Vector3f(tr.origin); // Stack.alloc(tr.origin);
        transform(vec);
    ...
    public void mul(final Transform tr1, final Transform tr2) {
        final Vector3f vec = new Vector3f(tr2.origin); // Stack.alloc(tr2.origin);
        tr1.transform(vec);
    ...
    public void invXform(final Vector3f inVec, final Vector3f out) {
        ...
        final Matrix3f mat = new Matrix3f(basis); // Stack.alloc(basis);
        mat.transpose();
        mat.transform(out);
    }

    public Quaternionf getRotation(final Quaternionf out) {
        basis.getNormalizedRotation(out);
        return out;
    }
...
//    }
    ...
    public boolean equals(final Object obj) {
        ...
        final Transform tr = (Transform) obj;
