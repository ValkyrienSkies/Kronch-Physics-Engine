...
public class HashedOverlappingPairCache extends OverlappingPairCache {
    ...
    public HashedOverlappingPairCache() {
        final int initialAllocatedSize = 2;
    ...
    public Object removeOverlappingPair(BroadphaseProxy proxy0, BroadphaseProxy proxy1, final Dispatcher dispatcher) {
        ...
        if (proxy0.getUid() > proxy1.getUid()) {
            final BroadphaseProxy tmp = proxy0;
            proxy0 = proxy1;
            proxy1 = tmp;
        }
        final int proxyId1 = proxy0.getUid();
        final int proxyId2 = proxy1.getUid();

		/*if (proxyId1 > proxyId2) 
		btSwap(proxyId1, proxyId2);*/

        final int hash = getHash(proxyId1, proxyId2) & (overlappingPairArray.capacity() - 1);

        final BroadphasePair pair = internalFindPair(proxy0, proxy1, hash);
        if (pair == null) {
            return null;
        }
        ...
        final Object userData = pair.userInfo;
        ...
        final int pairIndex = overlappingPairArray.indexOf(pair);
        assert (pairIndex != -1);
        ...
        final int lastPairIndex = overlappingPairArray.size() - 1;
        ...
        final BroadphasePair last = overlappingPairArray.getQuick(lastPairIndex);
        /* missing swap here too, Nat. */
        final int lastHash =
            getHash(last.pProxy0.getUid(), last.pProxy1.getUid()) & (overlappingPairArray.capacity() - 1);
    ...
    private BroadphasePair internalAddPair(BroadphaseProxy proxy0, BroadphaseProxy proxy1) {
        ...
        final int count = overlappingPairArray.size();
        final int oldCapacity = overlappingPairArray.capacity();
        ...
        final int newCapacity = overlappingPairArray.capacity();
    ...
    private void growTables() {
        ...
        if (hashTable.size() < newCapacity) {
            ...
            final int curHashtableSize = hashTable.size();
            ...
            for (int i = 0; i < curHashtableSize; i++) {
                ...
                final BroadphasePair pair = overlappingPairArray.getQuick(i);
                final int proxyId1 = pair.pProxy0.getUid();
                final int proxyId2 = pair.pProxy1.getUid();
				/*if (proxyId1 > proxyId2) 
				btSwap(proxyId1, proxyId2);*/
                final int hashValue =
                    getHash(proxyId1, proxyId2) & (overlappingPairArray.capacity() - 1); // New hash value with new mask
    ...
    private static class CleanPairCallback extends OverlapCallback {
        ...
        public CleanPairCallback(
            final BroadphaseProxy cleanProxy, final OverlappingPairCache pairCache, final Dispatcher dispatcher) {
